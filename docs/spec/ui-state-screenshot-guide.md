...
| Document | Agent comment working | Submit a comment with fake adapter status `working` | `agent-comment-task-button`, `document-comment-marker-c1-working` | Status label: `Agent working`; the anchored marker shows a spinner and the highlight uses the working color. |
...
| Comment markers | Working comment | Submit a comment while an agent adapter is connected | `document-comment-marker-c1-working`, `comment-rail-c1-working` | Marker shows a spinner while the submitted comment is being handed to the agent; the highlight changes to the working color until the state clears. |
