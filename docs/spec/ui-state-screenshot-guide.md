...
| Document | View mode, desktop | Open a plain or review fixture; View is the default | `document-mode-group`, `document-mode-view`, `document-page-header`, `document-content-card` | Eye is pressed in the neutral charcoal control at the viewport top-right. Capture below and above the 1504px review-rail breakpoint: the native title shows one basename, the workspace has no duplicate filename, the approximately 54rem reader stays horizontally centered and unboxed, and no review-rail space is reserved. In dark mode, the canvas is `#181818` and the scrollbar track blends into that same surface instead of producing a light strip. |
| Document | Comment unavailable, no agent | Open a local file without a watcher | `document-mode-comment-trigger`, `document-mode-comment` | Comment is disabled while View remains available. Hover the wrapper to capture the `No agent is reviewing this file` tooltip. |
| Document | Comment mode, agent connected | Open the review fixture while a watcher is connected, then click `document-mode-comment` | `document-mode-comment`, `document-mode-comment-agent-indicator`, `document-review-rail` | Comment is enabled with a small green connection dot. Once pressed, review controls are available. Use a viewport at least 1504px wide to capture the approximately 62rem main column beside the 24rem review rail. |
| Document | Comment mode, agent disconnected mid-review | Enter Comment while a watcher is connected, then disconnect the watcher | `document-mode-comment-trigger`, `document-mode-comment-agent-indicator`, `document-review-rail` | Preserve the active review and all drafts, change the connection dot to amber, and show `Agent disconnected — feedback is still saved`. After returning to View, Comment remains disabled until an agent reconnects. |
